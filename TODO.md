...
- [x] **In the reconcile pass (P0 — carries design decisions):**
  ...
  - [x] **Centralize the `confirm`/`idempotencyKey` merge.** *(DONE — `CONFIRM_FIELD`/`IDEMPOTENCY_FIELD` + the merge are now centralized in `@facet/surface-kit` and imported by both surfaces (`packages/mcp/src/tool.ts` + `packages/agent/src/index.ts`); no longer duplicated.)*
  - [ ] **Assert advertise == enforce.** Assert advertised-minus-merged(`confirm`/`idempotencyKey`) == enforced schema; flag `.refine()` cases. *(STILL OPEN — the parity matrix proves `validation` is ENFORCED uniformly across all five legs, but there is no explicit "advertised JSON Schema == enforced schema" assertion.)*
